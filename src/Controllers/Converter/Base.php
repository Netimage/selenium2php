...
class Base
{
    ...
    /**
     * Parses HTML table from test case object of the belonging commands,
    ...
    protected function _convertToTpl(string $tplFile, array $commandLines = null)
    {
        ...
        $replacements = array(
            ...
            '{$testMethodContent}' => $commandLines ? '' : $this->_composeStrWithIndents($this->_composeTestMethodContent($commandLines),
                8),
    ...
    protected function _createBrowsers()
    {
        ...
        $browsers = parse_ini_file(__DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . 'browsers.ini',
            true, INI_SCANNER_TYPED);

//		exit(var_dump($browsers));
        ...
        if (intval($this->browserstackLocal)) {
            ...
            if ($this->browserstackLocalIdentifier) {
                $capabilities .= $this->_createArrayParam('browserstack.localIdentifier',
                    $this->browserstackLocalIdentifier);
            }
        }
        ...
        $template = "array(
			...
			'desiredCapabilities'		=> array(
                ...
                'moz:firefoxOptions' => [
                    'prefs' => [
                        'geo.enabled' => {geoLocationEnabled}
                    ]
                ],
                'chromeOptions' => [
                    'prefs' => [
                        'profile.managed_default_content_settings.geolocation' => {geoLocationEnabled}
                    ]
                ]
			)
		)";
        ...
        foreach ($browserList as $browserName) {
            ...
            $browserArr[] = str_replace([
                ...
                $browser['osVersion'],
                $browser['resolution'],
                $browser['browserLocationPermission']
            ], $template);
        }
